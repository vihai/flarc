...
MONTH_IDS = [ "1", "2", "3", "4", "5", "6", "7", "8", "9", "A", "B", "C" ]
  ...
  def read_contents(options = {})
	...
	next if s[0..0] == '-'
        ...
        @b_ext.each do |ext|
          ...
          newpoint[B_EXTENSIONS[ext[:code]][:attr]] = s[ext[:start]-2..ext[:end]-1].send(B_EXTENSIONS[ext[:code]][:conv])
        end
	...
	# Ugly and raw takeoff/landing detection
        ...
        if (!prev1.nil?)
          begin
            distance = Math.acos(Math.sin(newpoint[:lat] / 180 * Math::PI)*Math.sin(prev1[:lat] / 180 * Math::PI) + 
                      Math.cos(newpoint[:lat] / 180 * Math::PI)*Math.cos(prev1[:lat] / 180 * Math::PI) *
                      Math.cos((newpoint[:lon] / 180 * Math::PI) - (prev1[:lon] / 180 * Math::PI))) * 6371000

          rescue Errno::EDOM
            # Happens when result is 1 which is rounded to something a bit greater than 1
            distance = 0
          end

          speed = distance / (newpoint[:logged_at] - prev1[:logged_at]);

#puts "P T#{newpoint[:logged_at]} A#{newpoint[:gnss_alt]} D#{distance} T#{newpoint[:logged_at] - prev1[:logged_at]} S#{speed * 3.6}\n"
        end
